...
class TestStructuredAskUserRailResolveInterrupt:
    ...
    #
    # Several channels post only the first question of a call: Slack, Feishu
    # and the CLI each read ``questions[0]`` and discard the rest, while Web
    # and the TUI render every one. The rail cannot tell which channel answered
    # it, and it does not have to: it knows what it asked and it can count what
    # came back. Without this the call reads as fully answered and the model
    # tells the user that every question was put to them.
