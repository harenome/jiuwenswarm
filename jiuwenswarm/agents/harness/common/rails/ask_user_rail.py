...
# The tightest cap any channel puts on one question's inputs. A renderer that
# knows Block Kit's ceiling refuses a longer question rather than posting a
# message it knows will be rejected, so the schema states the same number
# instead of letting the model write a question that cannot be shown.
...
# knob; neither exists. ``choice`` is not a type any renderer resolves, and
# ``multi`` is derived from an input's element types rather than declared, so
# the pair would have needed new specs to mean anything. ``select`` and
# ``multi_select`` are what already works, are equally expressible anywhere, and
# need nothing built.
#
# Twelve further types resolve in the Slack renderer and stay reachable for a
# question deliberately scoped to one platform -- ``datetime_unix``,
# ``user(s)``, ``conversation(s)``, ``channel(s)``, ``checkboxes``, ``radio``,
# ``email``, ``url``, ``rich_text``. They are not advertised. Six of them answer
# with Slack ids no other channel can produce, and a vocabulary is the expensive
# thing to withdraw: a name the model has been told about is one questions get
# written against. Adding to this tuple later is additive and costs nothing;
# removing from it breaks questions already in the wild.
